...
export default function BookAppointment() {
    ...
    const fetchData = async () => {
      try {
        const [pRes, dRes] = await Promise.all([
          fetch('http://localhost:5230/api/receptionist/patients'),
          fetch('http://localhost:5230/api/receptionist/appointments/doctors')
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:5230/api/receptionist/appointments', {
        method: 'POST',
